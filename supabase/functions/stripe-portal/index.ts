...
Deno.serve(async (req) => {
  ...
  if (req.method === "OPTIONS") {
   ...
   try {
    ...
    const { data: isTestUser } = await supabaseClient.rpc("is_internal_test_user", { p_user_id: userId });
    if (isTestUser) {
      logStep("Internal test user detected — skipping Stripe portal", { userId });
      return new Response(
        JSON.stringify({
